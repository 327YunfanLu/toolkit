    ...
    with open(pth) as file:
        for line in file:
            ...
            if line:
                ...
                dict_1[line[1].strip('"')] = line[0].strip('"')
    return dict_1

print(get_tics(TICPATH))
    ...
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            ...
            for tic in tickers_lst:
                if tic not in tic_exchange_dic.keys():
                    raise Exception
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for i in tickers_lst:
        dict_3[i] = {}
        dict_3[i]['exchange'] = tic_exchange_dic[i]
        dict_3[i]['data'] = []

        lines = read_dat(i)
        for line in lines:
            x = line_to_dict(line)
            x = {i: x[i] for i in x.keys() if i in col_lst}
            dict_3[i]['data'].append(x)
